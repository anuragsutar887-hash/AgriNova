(() => {
  ...
  // mic_entry.js
  var require_mic_entry = __commonJS({
    "mic_entry.js"() {
      init_esm();
      window.startCapacitorMic = async function(lang, callback, errorCallback) {
        try {
          const hasPerm = await SpeechRecognition2.hasPermission();
          if (!hasPerm.permission) {
            await SpeechRecognition2.requestPermission();
          }
          const perm = await SpeechRecognition2.hasPermission();
          if (perm.permission) {
            ...
            errorCallback("Please enable Microphone permission in your phone settings.");
          }
        } catch (e) {
          errorCallback("Microphone error: " + e.message);
        }
      };
    }
  });
